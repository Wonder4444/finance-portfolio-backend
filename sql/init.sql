...
CREATE TABLE `user_holding` (
                                ...
                                `is_deleted` BIGINT NOT NULL DEFAULT 0 COMMENT 'Soft delete: 0=active, else=deleted_id',
                                `create_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'Creation time',
                                `update_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT 'Update time',
                                PRIMARY KEY (`id`),
                                INDEX `idx_user_id` (`user_id`) COMMENT 'Performance index for user portfolio lookup',
                                UNIQUE KEY `uk_user_asset` (`user_id`, `asset_id`, `is_deleted`) COMMENT 'Ensures unique entry per active asset per user'
...
CREATE TABLE `user_watchlist` (
                                  ...
                                  `asset_id` BIGINT NOT NULL COMMENT 'Logical FK: Reference to asset_info.id',
                                   ...
                                   UNIQUE KEY `uk_user_asset_watch` (`user_id`, `asset_id`, `is_deleted`) COMMENT 'Ensures unique entry per active asset per user'
